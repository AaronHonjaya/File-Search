...
HashTableReader::HashTableReader(FILE* f, IndexFileOffset_t offset)
  ...
  Verify333(fread(&header_, sizeof(BucketListHeader), 1, f) == 1);
...
list<IndexFileOffset_t> HashTableReader::LookupElementPositions(
    HTKey_t hash_key) const {
  ...
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    ElementPositionRecord elem_rec;
    Verify333(fread(&elem_rec, sizeof(ElementPositionRecord), 1, file_) == 1);
    elem_rec.ToHostFormat();
    ret_val.push_back(elem_rec.position);
  }
  // Return the list.
  return ret_val;
}
